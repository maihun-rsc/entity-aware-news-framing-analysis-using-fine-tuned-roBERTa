...
Environment:
    The script auto-detects IDE vs Kaggle vs local CPU.
...
def stage_annotate(cfg: Config) -> int:
    ...
    return annotate_dataset(processed_path, annotated_path, limit=cfg.max_articles_per_outlet or 0)
...
def main() -> None:
    ...
    if stage in ("annotate", "all"):
        count = stage_annotate(cfg)
        if stage == "all" and count < 10:
            log.error(f"[main] Pipeline aborting: Only {count} articles annotated (minimum 10 required).")
            sys.exit(1)
