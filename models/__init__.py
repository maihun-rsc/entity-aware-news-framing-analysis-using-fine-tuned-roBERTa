...
def run_training(cfg: "Config") -> dict:
    ...
    examples = []
    for art in labeled:
        text = art.clean_body or art.body
        label = art.label
        target_ent = art.entities[0] if art.entities else None
        
        words = text.split()
        seq_len = len(words)
        if target_ent and seq_len > 0:
            ent_tokens = [t.lower() for t in target_ent.split() if len(t) > 2]
            ent_indices = [i for i, w in enumerate(words) if any(t in w.lower() for t in ent_tokens)]
            if ent_indices:
                prox = [1.0 / (1.0 + min(abs(i - e_idx) for e_idx in ent_indices)) for i in range(seq_len)]
            else:
                prox = [1.0] * seq_len
        else:
            prox = [1.0] * max(1, seq_len)
            
        examples.append(FramingExample(text, prox, label))
